...
// and the vtable pointer (!cir.vptr) are 4 bytes wide. The CIR record layout
// derives that width from the data layout, so the field following a pointer
// lands at the AST-mandated offset. Sizing pointers as a hardcoded 64 bits
// previously tripped the record layout builder (insertPadding: assertion
// `offset >= size`) on every record containing a pointer for such targets.
...
// The 4-byte pointer is immediately followed by 'x' at offset 4 with no
// padding, and each record is 4-byte aligned.
